...
#### Node / Vertex
Elements of data in the graph, typically represented as circles
...
class Node:
    def __init__(self, data):
        self.data = data
        self.next = None  # An edge
        self.prev = None  # An edge
```
...
##### Simple Graph
A graph that has neither loop nor parallel edges and each edge connects two different vertices
...
##### Finite Graph
A graph with a finite number of vertices and finite number of edges
![[Pasted image 20220418160808.png |300]]

##### Infinite Graph
A graph without a finite number of vertices and edges
![[Pasted image 20220418160822.png |300]]

### Graph Terminology

##### Adjacent / Neighbors
Two vertices connected by an edge. A vertex is adjacent to itself if connected by a loop.
...
##### Regular Graph
If all the vertices of a graph $G$ have the same degree $d$, then $G$ is called a "d-regular-graph"
...
##### Adjacency List
A list for directed or undirected graphs where you acknowledge the vertices connected to the edges of another vertex
...
If $G$ is a simple graph with $n$ vertices where $n \ge 3$ and for every pair of nonadjacent vertices $u$ and $v$ in $G, deg(u) + deg(v) \ge n$, then $G$ has a Hamilton circuit.
![[Pasted image 20220424135027.png |200]]


##### Petersen Graph
An undirected graph with 10 vertices and 15 edges, typically drawn as a pentagram within a pentagon
...
> Is $K_5$ non-planar?
> **ANS:** 
> 1. We know that graph $K_5$ has five vertices and 10 edges
> 2. Using Corollary 2, we can say that $3 \cdot 5 - 6 = 9$
> 3. We know that $10 \not \le 9$ 
...
##### Corollary 4
If a connected planar simple graph $G$ has $e$ edges and $v$ vertices where $v \ge 3$ and there is no circuit of length three, then $e \le 2v -4$

> Determine whether $K_{3,3}$ is planar or non-planar by Corollary 4
> **ANS:** 
> Graph $K_{3,3}$ is bipartite graph then it has no circuits of length three.
> We know that there are six vertices and nine edges in $K_{3,3}$
...
![[Pasted image 20220509235548.png|600]] 
...
##### Child
The vertices connected beneath a vertex
...
##### Level of Tree
How many parent vertices separate the current node from the root
...
A tree organized by
1. Left subtree only contains nodes lesser than itself
2. Right subtree only contains nodes greater than itself
3. The left and right subtrees  must maintain rules 1 and 2
![[Pasted image 20220424182234.png |400]]
##### Example :: Binary search tree using words:
> Construct a binary search tree for the words: “the quick brown fox jumps over the lazy dog”
...
> A decision tree is used to sort three distinct elements x, y, z
> **ANS:**
...
A way to order the vertices of an ordered rooted tree using recursion.
...
1. Visit left sub-trees recursively by inorder
2. Visit root
3. Visit right sub-trees recursively preorder
...
#### Tree Expression
Mathematical expressions can be expressed using trees
